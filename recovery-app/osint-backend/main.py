...
async def scrape_jail_roster(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        errors.append(f"Cloudscraper attempt: {str(e)}")

    # Method 2: Try curl_cffi (impersonates Chrome TLS fingerprint)
    if not html:
        try:
            ...
            errors.append(f"Curl-cffi attempt: {str(e)}")
    ...
    if not html:
        try:
            ...
            errors.append(f"Httpx attempt: {str(e)}")
    ...
    if not html:
        try:
            ...
            async with async_playwright() as p:
                ...
                response_status = 200  # If we got here, it loaded

                await browser.close()
        except Exception as e:
            errors.append(f"Playwright attempt: {str(e)}")

    # If all methods failed
    if not html:
        if response_status == 403:
            errors.append(f"Site blocked all access methods (HTTP 403). Try viewing manually.")
            inmate['_manual_entry_required'] = True
            inmate['_source_url'] = url
            inmate['_booking_id'] = booking_id
        else:
            errors.append(f"Failed to fetch page: HTTP {response_status}")
        return {
            'url': url,
            ...
            'photo_url': photo_url,
            'errors': errors,
            'manual_entry_hint': f"Open {url} in your browser to view inmate details.",
            'execution_time': (datetime.now() - start_time).total_seconds()
        }
